...
def train_solver_sci(single_iterate_solver, train_dataloader, optimizer,
                 ...
                 print_every_n_steps=100, save_every_n_steps=10, save_best_every_n_steps=100, start_epoch=0,
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for ii, sample_batch in tqdm(enumerate(train_dataloader)):
            ...
            Phi_sum[Phi_sum == 0] = 1
            with torch.no_grad():
                initial_point = cg_utils.initial_point_admm(y, Phi, Phi_sum, gt_batch) # debug: .to(device=device)
            ...
            if ii % save_best_every_n_steps == 0:
...
def test_solver_sci(deep_eq_module, test_dataloader=None, save_img_path=None, verbose=True, save_image=True):
    ...
    for ii, sample_batch in enumerate(test_dataloader):
        ...
        for fi in range(f):
            ...
            with torch.no_grad():
                initial_point = cg_utils.initial_point_admm(y, Phi, Phi_sum, gt_batch)
