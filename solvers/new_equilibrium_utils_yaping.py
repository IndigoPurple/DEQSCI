...
def andersonexp(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-5, beta=1.0): # yaping mute
...
class DEQFixedPoint(nn.Module):
    ...
    def forward(self, x, Phi, Phi_sum, initial_point = None, train_flag=True):
        ...
        # if isinstance(self.f, torch.nn.DataParallel):
        #     self.f = self.f.module
        # is_training = self.f.nonlinear_op.training
        # self.f.nonlinear_op.eval()
        with torch.no_grad():
            '''solver is anderson, 
            ...
            z, self.forward_res = self.solver(lambda z: self.f(z, x, Phi, Phi_sum), init_point, **self.kwargs)
        # self.f.nonlinear_op.train(mode=is_training)

        z = self.f(z, x, Phi, Phi_sum)

        # set up Jacobian vector product (without additional forward calls)
        z0 = z.clone().detach().requires_grad_()
        f0 = self.f(z0, x, Phi, Phi_sum)
...
def admmexp(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-2, beta=1.0): # yaping mute
# def andersonexp(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-2, beta=1.0): #yaping add
    """ ADMM fixed point iteration. """
    # global tt
    # bsz, H, W, d = x0[0].shape

    X, U = x0[0], x0[1]
    for k in range(2, max_iter):
        new_X, new_U = f(X,U)
        res = (new_X- X).norm().item() / (1e-5 + new_X.norm().item())
...
class DEQFixedPointADMM(nn.Module):
    def __init__(self, f, solver1, solver2, **kwargs):
        super().__init__()
        self.f = f
    ...
    def forward(self, x, Phi, Phi_sum, initial_point=None, train_flag=True):
        if initial_point is None:
            init_point = [torch.zeros_like(x), torch.zeros_like(x)]
        else:
            init_point = initial_point
        ...
        # with torch.no_grad():
        #     '''solver is anderson,
        #         f is EquilibriumADMMSCI,
        #         init_point is At(y,Phi),
        #         x is y, measurement'''
        #     '''aderson(EquilibriumProxGradSCI, init_point, args)'''
        #     z, u, self.forward_res = self.solver1(lambda z, u: self.f(z, u, x, Phi, Phi_sum), init_point, **self.kwargs)
        #
        # z, u = self.f(z, u, x, Phi, Phi_sum)

        # set up Jacobian vector product (without additional forward calls)
        # z0 = z.clone().detach().requires_grad_()
        # u0 = u.clone().detach().requires_grad_()
        # f0, f1 = self.f(z0, u0, x, Phi, Phi_sum)
        #
        # def backward_hook(grad):
        #     g, self.backward_res = self.solver2(lambda y: torch.autograd.grad(f0, z0, y, retain_graph=True)[0] + grad,
        #                                        grad, **self.kwargs)
        #     return g
        # z.requires_grad_()
        # z.register_hook(backward_hook)
